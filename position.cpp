...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  if(is_atomic())
  {
  	uint64_t tk=k & Eval::THEORY_MASK;

  	Eval::theoryItem ti=Eval::atomictheoryItems[tk];
  	if(ti.key==k)
  	{
  		if((st->goodForWhite==0)&&(st->goodForBlack==0))
  		{
	  		st->goodForWhite=ti.goodForWhite;
	  		st->goodForBlack=ti.goodForBlack;
  		}
  	}
  }
#endif
...
Value Position::getTheoryValue() const {
	return (Value)( (st->goodForWhite-st->goodForBlack) * THEORY_BONUS );
}

#ifdef ATOMIC
  Value Position::getAtomicPieceBonuses() const {
    Value v=VALUE_ZERO;
    for(Color c=WHITE; c<=BLACK; ++c)
    {
      Bitboard qbb=byTypeBB[QUEEN] & byColorBB[c];
      while(qbb) { v += ( 1 - 2*c ) * ATOMIC_QUEEN_BONUS; pop_lsb(&qbb); }
      Bitboard pbb=byTypeBB[PAWN] & byColorBB[c];
      while(pbb) { v += ( 1 - 2*c ) * ATOMIC_PAWN_BONUS; pop_lsb(&pbb); }
    }
    return v;
  }
#endif

void Position::nulloutTheory() const {
	st->goodForWhite=0;
	st->goodForBlack=0;
}
...
Eval::theoryItem* Position::lookUpTheory() const {
  uint64_t tk=key() & Eval::THEORY_MASK;

  if( Eval::theoryItems[tk].key == key() ) return &Eval::theoryItems[tk];

  return nullptr;
}

int Position::goodForWhite() const {
  Eval::theoryItem* tip=lookUpTheory();
  if(tip != nullptr ) return tip -> goodForWhite;
  return 0;
}

int Position::goodForBlack() const {
  Eval::theoryItem* tip=lookUpTheory();
