...
#ifdef ATOMIC
int ATOMIC_MOBILITY_BONUS     = 11;
int ATOMIC_KING_ATTACK_BONUS  = 20;
Score ATOMIC_PAWN_BONUS       = (Score)363;
Score ATOMIC_QUEEN_BONUS      = (Score)502;
#endif

int THEORY_BONUS              = (Value)50;
...
namespace {
  ...
  Score evaluate_pieces(const Position& pos, EvalInfo& ei, Score* mobility,
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        if(pos.is_atomic())
        {
          ...
          mobility[Us] += ATOMIC_MOBILITY_BONUS * MobilityBonus[Pt][mob];
  ...
  Score evaluate_king(const Position& pos, const EvalInfo& ei) {
    ...
    if (ei.kingAttackersCount[Them])
    {
        ...
        if(pos.is_atomic())
        {
        	attackUnits += ATOMIC_KING_ATTACK_BONUS * ei.kingAdjacentZoneAttacksCount[Them];
        }
#endif
...
Value Eval::evaluate(const Position& pos) {
  ...
  if(pos.is_atomic())
  {
    v += pos.getTheoryValue() + pos.getAtomicPieceBonuses();
  }
#endif
...
namespace Eval {
  ...
  std::string uint64_t_to_string(uint64_t u)
  {
    ...
    do
    {
      ...
      switch(mod)
      {
        case 0: buff="0"+buff;break;
        case 1: buff="1"+buff;break;
        case 2: buff="2"+buff;break;
        case 3: buff="3"+buff;break;
        case 4: buff="4"+buff;break;
        case 5: buff="5"+buff;break;
        case 6: buff="6"+buff;break;
        case 7: buff="7"+buff;break;
        case 8: buff="8"+buff;break;
        case 9: buff="9"+buff;break;
      }
    } while(u>0);
    return buff;
  }
  ...
  theoryItem theoryItems[THEORY_MASK+1];

  void loadTheory(std::string filename)
  {
    std::memset(theoryItems, 0, sizeof(theoryItems));
    ...
    if (myfile.is_open())
    {
      while ( getline (myfile,line) )
      {
        ...
        is >> token;
        uint64_t key=string_to_uint64_t(token);
        is >> token;
        int goodForWhite=std::stoi(token);
        is >> token;
        int goodForBlack=std::stoi(token);
        is >> token;
        int ply=std::stoi(token);
        theoryItems[index].key=key;
        theoryItems[index].goodForWhite=goodForWhite;
        theoryItems[index].goodForBlack=goodForBlack;
  ...
  void saveTheory(std::string filename)
  {
    std::ofstream myfile;
    myfile.open (filename+".txt");
    for(int i=0; i<=(int)THEORY_MASK; i++)
    {
      uint64_t key=theoryItems[i].key;
      int goodForWhite=theoryItems[i].goodForWhite;
      int goodForBlack=theoryItems[i].goodForBlack;
      int ply=theoryItems[i].ply;
      if((goodForWhite!=0)||(goodForBlack!=0))
      {
        myfile << i << " " << uint64_t_to_string(key) << " " << goodForWhite << " " << goodForBlack << " " << ply << std::endl;
      }
    }
    myfile.close();
    myfile.open (filename+".cpp");
    myfile << "#include \"evaluate.h\"" << std::endl << std::endl;
    myfile << "#include <cstring>" << std::endl << std::endl;
    myfile << "namespace Eval {" << std::endl << std::endl;
    myfile << "\ttheoryItem " << filename << "Items[THEORY_MASK+1];" << std::endl << std::endl;
    myfile << "\tvoid initatomictheory() {" << std::endl << std::endl;
    myfile << "\t\tstd::memset(atomictheoryItems, 0, sizeof(atomictheoryItems));" << std::endl << std::endl;
    for(int i=0; i<=(int)THEORY_MASK; i++)
    {
      uint64_t key=theoryItems[i].key;
      int goodForWhite=theoryItems[i].goodForWhite;
      ...
      if((goodForWhite!=0)||(goodForBlack!=0))
      {
        myfile << "\t\t" << filename << "Items[" << i << "].key=" << uint64_t_to_string(key) << "ULL;" << std::endl;
        myfile << "\t\t" << filename << "Items[" << i << "].goodForWhite=" << goodForWhite << ";" << std::endl;
        myfile << "\t\t" << filename << "Items[" << i << "].goodForBlack=" << goodForBlack << ";" << std::endl;
        myfile << "\t\t" << filename << "Items[" << i << "].ply=" << ply << ";" << std::endl;
      }
    }
    myfile << std::endl;
  ...
  bool theoryItem::free() const {
    return ( goodForWhite == 0 ) && ( goodForBlack == 0 );
  }
